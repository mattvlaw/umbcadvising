<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf8">
    <link rel="stylesheet" href="classList.css">
    <title>Computer Science Class List</title>
  </head>
  <body>
    <a href="http://umbc.edu"><img class="header" src="./img/logo.jpg"></a>
    <!-- Credit Counter -->
    <div id="credcounter"></div>
      <form action="" id="classlist">
    
    <div id="userinfo">
      <label for="name">Name: </label>
      <input type="text" name="name" id="name" required>
      <label for="email">Best Email: </label>
      <input type="email" name="email" id="email" required>
      <label for="contactnum">Best Contact Number: </label>
      <input type="text" name="contactnum" id="connectnum" required>
      <label for="campusid">Best Campus ID: </label>
      <input type="text" name="campusid" id="campusid" required>
    </div>

    <h1>Undergraduate Course Advising</h1>
      <!-- class path tree -->
      <div id="classtree"> 

	<!-- computer science path trees -->
	<div id="compsci">
	  <div id="cs200"> 
	    <!-- CMSC 200 level courses -->
	    <h2> CMSC 200 Level Courses</h2>
            <label class="course" id="cmsc201label" for="cmsc201" onclick="labelOnClick(this)" >CMSC 201</label>
	    <input type="checkbox" class="" name="course[]" id="cmsc201" value="cmsc201">
	    <label class="course" id="cmsc202label" for="cmsc202" onclick="labelOnClick(this)">CMSC 202</label>
            <input type="checkbox" class="" name="course[]" id="cmsc202" value="cmsc202" >
	    <label class="course" for="cmsc203" onclick="labelOnClick(this)">CMSC 203</label>
            <input type="checkbox" class="" name="course[]" id="cmsc203" value="cmsc203">
	    <label class="course" for="cmsc232" onclick="labelOnClick(this)">CMSC 232</label>
            <input type="checkbox" class="" name="course[]" id="cmsc232" value="cmsc232">
	    <label class="course" for="cmsc291" onclick="labelOnClick(this)">CMSC 291</label>
            <input type="checkbox" class="" name="course[]" id="cmsc291" value="cmsc291">
	    <label class="course" for="cmsc299" onclick="labelOnClick(this)">CMSC 299</label>
            <input type="checkbox" class="" name="course[]" id="cmsc299" value="cmsc299">
	  </div>
	  <br>
	  
	  <div id="cs300"> 
	    <!-- CMSC 300 level courses -->
	    <h2>CMSC 300 Level Courses</h2>
            <label class="course" for="cmsc304">CMSC 304</label> 
	    <input type="checkbox" class="" name="course[]" id="cmsc304" value="cmsc304">
	    <label class="course" for="cmsc313">CMSC 313</label>
            <input type="checkbox" class="" name="course[]" id="cmsc313" value="cmsc313">
	    <label class="course" for="cmsc331">CMSC 331</label>
            <input type="checkbox" class="" name="course[]" id="cmsc331" value="cmsc331">
	    <label class="course" for="cmsc341">CMSC 341</label>
            <input type="checkbox" class="" name="course[]" id="cmsc341" value="cmsc341">
	    <label class="course" for="cmsc352">CMSC 352</label>
            <input type="checkbox" class="" name="course[]" id="cmsc352" value="cmsc352">
	    <label class="course" for="cmsc391">CMSC 391</label>
            <input type="checkbox" class="" name="course[]" id="cmsc391" value="cmsc391">
	  </div>
          <br>

	  <div id="cs400"> 
	    <!-- CMSC 400 level courses -->
	    <h2>CMSC 400 Level Courses</h2>
            <label class="course" for="cmsc404">CMSC 404</label>
	    <input type="checkbox" class="" name="course[]" id="cmsc404" value="cmsc404">
            <label class="course" for="cmsc411">CMSC 411</label>
            <input type="checkbox" class="" name="course[]" id="cmsc411" value="cmsc411">
            <label class="course" for="cmsc421">CMSC 421</label>
            <input type="checkbox" class="" name="course[]" id="cmsc421" value="cmsc421">
            <label class="course" for="cmsc426">CMSC 426 </label>         
            <input class="course" type="checkbox" class="" name="course[]" id="cmsc426" value="cmsc426">
            <label class="course" for="cmsc427">CMSC 427  </label>
            <input type="checkbox" class="" name="course[]" id="cmsc427" value="cmsc427">
            <label class="course" for="cmsc431">CMSC 431</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc431" value="cmsc431">
            <label class="course" for="cmsc432">CMSC 432</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc432" value="cmsc432">
            <label class="course" for="cmsc433">CMSC 433</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc433" value="cmsc433">
            <label class="course" for="cmsc435">CMSC 435</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc435" value="cmsc435">
            <label class="course" for="cmsc436">CMSC 436</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc436" value="cmsc436">
            <label class="course" for="cmsc437">CMSC 437</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc437" value="cmsc437">
            <label class="course" for="cmsc441">CMSC 441</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc441" value="cmsc441">
            <label class="course" for="cmsc442">CMSC 442</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc442" value="cmsc442">
            <label class="course" for="cmsc443">CMSC 443</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc443" value="cmsc443">
            <label class="course" for="cmsc446">CMSC 446</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc446" value="cmsc446">
            <label class="course" for="cmsc447">CMSC 447</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc447" value="cmsc447">
            <label class="course" for="cmsc448">CMSC 448</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc448" value="cmsc448">
            <label class="course" for="cmsc451">CMSC 451</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc451" value="cmsc451">
            <label class="course" for="cmsc452">CMSC 452</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc452" value="cmsc452">
            <label class="course" for="cmsc453">CMSC 453</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc453" value="cmsc453">
            <label class="course" for="cmsc455">CMSC 455</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc455" value="cmsc455">
            <label class="course" for="cmsc456">CMSC 456</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc456" value="cmsc456">
            <label class="course" for="cmsc457">CMSC 457</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc457" value="cmsc457">
            <label class="course" for="cmsc461">CMSC 461</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc461" value="cmsc461">
            <label class="course" for="cmsc465">CMSC 465</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc465" value="cmsc465">
            <label class="course" for="cmsc466">CMSC 466</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc466" value="cmsc466">
            <label class="course" for="cmsc471">CMSC 471</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc471" value="cmsc471">
            <label class="course" for="cmsc473">CMSC 473</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc473" value="cmsc473">
            <label class="course" for="cmsc475">CMSC 475</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc475" value="cmsc475">
            <label class="course" for="cmsc476">CMSC 476</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc476" value="cmsc476">
            <label class="course" for="cmsc477">CMSC 477</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc477" value="cmsc477">
            <label class="course" for="cmsc478">CMSC 478</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc478" value="cmsc478">
            <label class="course" for="cmsc479">CMSC 479</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc479" value="cmsc479">
            <label class="course" for="cmsc483">CMSC 483</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc481" value="cmsc481">
            <label class="course" for="cmsc483">CMSC 483</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc483" value="cmsc483">
            <label class="course" for="cmsc484">CMSC 484</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc484" value="cmsc484">
            <label class="course" for="cmsc486">CMSC 486</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc486" value="cmsc486">
            <label class="course" for="cmsc487">CMSC 487</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc487" value="cmsc487">
            <label class="course" for="cmsc491">CMSC 491</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc491" value="cmsc491">
            <label class="course" for="cmsc493">CMSC 493</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc493" value="cmsc493">
            <label class="course" for="cmsc495">CMSC 495</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc495" value="cmsc495">
            <label class="course" for="cmsc498">CMSC 498</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc498" value="cmsc498">
            <label class="course" for="cmsc499">CMSC 499</label>  
            <input type="checkbox" class="" name="course[]" id="cmsc499" value="cmsc499">
	  </div>
	</div>
			
	<!-- path trees for the suggested science courses -->
	<div id="sciences">
	  <div id="path1"> 
	    <!-- chem101 -> chem102 -> chem102L -> GES110 -->  
	  </div>
	  <div id="path2">
	    <!-- chem101 -> chem102 -> biol141 -> any lab -->
	  </div>
	  <div id="path3">
	    <!-- biol 141 -> biol142 -> biol Lab -> phys121 -->
	  </div>
	  <div id="path4">
	    <!-- phys121 -> phsy122 -> ges286 -->
	  </div>
	  <div id="path5">
	    <!-- phys121 -> phys122 -> phys 122L -> math251  -->
	  </div>
	  <div id="path6">
	    <!-- sci -> sci -> ges110/120 -> sci101L -->
	  </div>
	</div>
      </div>
      </form>
      <script type="text/javascript"  src="./classList.js"></script>
  </body>
</html>
